...
When Home is configured with the default `README.md` source, or its configured source is missing or invalid, Home contributes `home:create-home-page` to Resonance Actions. The Task owns its bounded documentation discovery policy and excludes dependencies, generated directories, repository internals, credentials, and unreadable files. It gives a package-owned Home agent the bounded documents and curation skill, then requires explicit confirmation before atomically writing `.resonance/home.html` and updating the Home package source. Existing custom sources are never overwritten. Runtime validation re-reads the configured HTML source through the same containment rules used by Home; completion is evidence-based and dismissal is stored separately in package state.

The Task uses the generic Resonance Actions session but owns the Home agent runtime. The runtime mounts `skills/create-home-page/SKILL.md` and the package's `home.css` as its design reference, exposes only the bounded documentation selected by the Task, and stages an HTML preview through a proposal tool. Host validation rejects inline styles, external stylesheets, executable content, and malformed heading structure, so applied pages use the package-owned design system. It does not receive shell, network, credentials, or unrestricted filesystem access. After confirmation, the Task completes to `/`, so Shell opens the newly configured Home surface. Configure the optional `provider` and `model` inputs on Home to select the OpenAI-compatible model; the runtime reads the matching API key from the process environment or from the viewed repository's gitignored `.resonance/home-agent.env`. The file must contain exactly `OPENROUTER_API_KEY=<key>` (or the matching OpenAI variable), be located under the Resonance application checkout (alongside the other package credential files), and have mode `0600`.
