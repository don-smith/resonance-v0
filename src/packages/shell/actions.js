export default function createActionsPackage({ fetchFn = fetch, eventSourceFactory = (url) => new EventSource(url), initialTasks = [], onTasksChanged = () => {} } = {}) {
  ...
  function renderState() {
    ...
    if (state.preview) { const heading = document.createElement('h3'); heading.textContent = state.preview.title; const summary = document.createElement('p'); summary.textContent = state.preview.summary || ''; const paths = document.createElement('p'); paths.textContent = `Affected: ${state.preview.affectedPaths.join(', ')}`; preview.append(heading, summary, paths); if (state.preview.content) { const pre = document.createElement('pre'); pre.textContent = state.preview.content; preview.append(pre); } if (state.pendingConfirmation) { const confirm = document.createElement('button'); confirm.type = 'button'; confirm.className = 'actions-confirm'; confirm.textContent = 'Apply changes'; confirm.addEventListener('click', confirmPreview); preview.append(confirm); } }
