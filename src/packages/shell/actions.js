export default function createActionsPackage({ fetchFn = fetch, eventSourceFactory = (url) => new EventSource(url), initialTasks = [], onTasksChanged = () => {} } = {}) {
  let root;
  ...
  function render() {
    ...
    root.innerHTML = `<div class="actions-workspace"><aside class="actions-list"><p class="eyebrow">RESONANCE ACTIONS</p><h2>Actions</h2><div class="actions-items"></div></aside><section class="actions-main"><header class="actions-header"><div><p class="actions-package"></p><h1 class="actions-title"></h1><p class="actions-description"></p><p class="actions-status"></p></div><div class="actions-controls"><button type="button" class="actions-dismiss">Dismiss</button><button type="button" class="actions-reset">New attempt</button></div></header><div class="actions-transcript"></div><div class="actions-preview" hidden></div><div class="actions-validation" hidden></div><form class="actions-composer"><textarea rows="3" placeholder="Describe what you want this action to do…"></textarea><div><button type="button" class="actions-stop" hidden>Stop</button><button type="submit" class="actions-send">Prepare</button></div></form></section></div>`;
    const list = root.querySelector('.actions-items');
    for (const item of tasks) { const button = document.createElement('button'); button.type = 'button'; button.className = `actions-item${item.id === selectedTask ? ' active' : ''}`; button.dataset.taskId = item.id; const label = document.createElement('span'); label.className = 'actions-item-label'; label.textContent = item.label; const owner = document.createElement('small'); owner.textContent = item.packageLabel; button.append(label, owner); list.append(button); }
  ...
  function renderState() {
    ...
    if (state.preview) { const heading = document.createElement('h3'); heading.textContent = state.preview.title; const summary = document.createElement('p'); summary.textContent = state.preview.summary || ''; const paths = document.createElement('p'); paths.textContent = `Affected: ${state.preview.affectedPaths.join(', ')}`; preview.append(heading, summary, paths); if (state.preview.configuration) { const configuration = document.createElement('p'); configuration.textContent = `Configuration: ${JSON.stringify(state.preview.configuration)}`; preview.append(configuration); } if (state.preview.content) { const frame = document.createElement('iframe'); frame.className = 'actions-preview-frame'; frame.title = 'Rendered Task preview'; frame.setAttribute('sandbox', ''); frame.srcdoc = `<link rel="stylesheet" href="/assets/home/home.css"><main>${state.preview.content}</main>`; preview.append(frame); const details = document.createElement('details'); const summary = document.createElement('summary'); summary.textContent = 'View HTML source'; const pre = document.createElement('pre'); pre.textContent = state.preview.content; details.append(summary, pre); preview.append(details); } if (state.pendingConfirmation) { const confirm = document.createElement('button'); confirm.type = 'button'; confirm.className = 'actions-confirm'; confirm.textContent = 'Apply changes'; confirm.addEventListener('click', confirmPreview); preview.append(confirm); } }
    ...
    const composer = root.querySelector('.actions-composer'); if (composer) composer.querySelector('textarea').disabled = state.status === 'in-progress';
  ...
  async function confirmPreview() { await request('/api/actions/confirm', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ taskId: selectedTask, confirmationId: state.pendingConfirmation.id }) }); await loadTasks(); }
